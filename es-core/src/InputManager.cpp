...
bool InputManager::parseEvent(const SDL_Event& ev, Window* window)
{
  ...
  case SDL_JOYAXISMOTION:
    int normValue;
    if(mInputConfigs[ev.jaxis.which]->isAxisConfigured(ev.jaxis.axis)) {
      if(ev.jaxis.value <= mInputConfigs[ev.jaxis.which]->getAxisNegativeThreshold(ev.jaxis.axis)) {
        normValue = -1;
      } else if(ev.jaxis.value <= mInputConfigs[ev.jaxis.which]->getAxisNegativeThreshold(ev.jaxis.axis)) {
        normValue = 1 ;
      } else {
        normValue = 0;
      }
    } else {
      normValue = ev.jaxis.value;
    }
    if(mPrevAxisValues[ev.jaxis.which][ev.jaxis.axis] != normValue) {
      window->input(getInputConfigByDevice(ev.jaxis.which), Input(ev.jaxis.which, TYPE_AXIS, ev.jaxis.axis, normValue, false));
      causedEvent = true;
    }

    mPrevAxisValues[ev.jaxis.which][ev.jaxis.axis] = normValue;
    return causedEvent;
